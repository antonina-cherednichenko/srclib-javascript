...
function analyseAll() {
    ...
    var searchVisitor = walk.make({
        ...
        ImportDeclaration: function(node, nodeInfo, c) {
            for (var i = 0; i < node.specifiers.length; ++i) {
                if (node.specifiers[i].local) {
                    ...
                    var fnDef = scope.getFunction(specifier.name);
                    if (fnDef) {
                        console.error("FN def = ", fnDef);
                        processIdent(specifier, nodeInfo, specifier.name, fnDef.path);
                        continue;
                    }
        ...
        JSXElement: function(node, nodeInfo, c) {
            ...
            var fnDef = scope.getFunction(nameNode.name);
            if (fnDef) {
                processIdent(nameNode, nodeInfo, nameNode.name, fnDef.path);
            }
