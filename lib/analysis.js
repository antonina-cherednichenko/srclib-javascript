...
function initTernServer(files) {
    ...
    logger.log('debug', "All ids = ", allIdents, " Undefined ids = ", undefinedIdents);
    if (allIdents != 0) {
        logger.log('debug', "percentage of undefined ids = ", (undefinedIdents / allIdents) * 100, "%");
    }
...
function getQueryInfo(file, offset, type, start) {
    ...
    ternServer.request({
        ...
        if (error) {
            logger.log('error', "Error returned from Tern 'definition' request: " + error);
            return;
        }
        res = data;
    });
...
function getExternalRepoInfo(data) {
    ...
    if (packageJson.repository === undefined) {
        logger.log('debug', "PACKAGE json does not define repository ", packageJson);
        return null;
    }
    ...
    var searchVisitor = walk.make({
        ...
        Identifier: function(node, kind, c) {
            ...
            if (data === null || pathForDef === null && data.url === undefined) {
                ...
                logger.log('debug', "EMPTY data for Identifier = ", node.name, "IN FILE = ", node.sourceFile.name);
                return;
            }
            ...
            if (pathForDef === pathForId || ((kind === "var") && (pathForDef !== pathForId))) {
                ...
                if (scopePathForId === null || scopePathForId === undefined) {
                    undefinedIdents = undefinedIdents + 1;
                    logger.log('debug', "SOURCE FILE = ", node.sourceFile.name);
                    logger.log('debug', "ERROR OCCURRED IN MAPPING SCOPE ID PATH", pathForId, "NAME=", node.name);
                    return;
                }
                var defData = {
                    Type: typeInfo.type,
                ...
                if (scopePathForDef === null || scopePathForDef === undefined) {
                    ...
                    undefinedIdents = undefinedIdents + 1;
                    logger.log('debug', "ERROR OCCURRED IN MAPPING SCOPE DEF PATH for", pathForDef, "NAME=", node.name);
                    logger.log('debug', "SOURCE FILE = ", node.sourceFile.name);
                    logger.log('debug', "DATA = ", data);
                    logger.log('debug', "External repo = ", externalRepo);
                    logger.log('debug', "===================================");
                    return;
                }
                var ref = {
